...
type HelloAPI struct {
	...
	HelloPointersEnconding
}

// HelloPointersEnconding encodes pointers to Struct and
// reverse pointers of slice of poitners to Struct
type HelloPointersEnconding struct {
...
// BackRepoHello.Init set up the BackRepo of the Hello
func (backRepoHello *BackRepoHelloStruct) Init(db *gorm.DB) (Error error) {

	if backRepoHello.Map_HelloDBID_HelloPtr != nil {
		err := errors.New("In Init, backRepoHello.Map_HelloDBID_HelloPtr should be nil")
		return err
	}

	if backRepoHello.Map_HelloDBID_HelloDB != nil {
		err := errors.New("In Init, backRepoHello.Map_HelloDBID_HelloDB should be nil")
		return err
	}

	if backRepoHello.Map_HelloPtr_HelloDBID != nil {
		err := errors.New("In Init, backRepoHello.Map_HelloPtr_HelloDBID should be nil")
		return err
	}

	tmp := make(map[uint]*models.Hello, 0)
	backRepoHello.Map_HelloDBID_HelloPtr = &tmp

	tmpDB := make(map[uint]*HelloDB, 0)
	backRepoHello.Map_HelloDBID_HelloDB = &tmpDB

	tmpID := make(map[*models.Hello]uint, 0)
	backRepoHello.Map_HelloPtr_HelloDBID = &tmpID
...
func (backRepoHello *BackRepoHelloStruct) CheckoutPhaseOne() (Error error) {
	...
	for key, value := range models.Stage.Hellos {
...
func (backRepoHello *BackRepoHelloStruct) CheckoutPhaseTwoInstance(backRepo *BackRepoStruct, helloDB *HelloDB) (Error error) {
	...
	_ = hello // sometimes, there is no code generated. This lines voids the "unused variable" compilation error
	...
	if _, ok := (*backRepo.BackRepoHello.Map_HelloPtr_HelloDBID)[hello]; ok {
		...
		if id, ok := (*backRepo.BackRepoHello.Map_HelloPtr_HelloDBID)[hello]; ok {
			...
			if err := backRepo.BackRepoHello.db.First(&helloDB, id).Error; err != nil {
				log.Panicln("CheckoutHello : Problem with getting object with id:", id)
			}
...
func (backRepoHello *BackRepoHelloStruct) Backup(dirPath string) {
	...
	if err != nil {
		log.Panic("Cannot json Hello ", filename, " ", err.Error())
	}

	err = ioutil.WriteFile(filename, file, 0644)
	if err != nil {
		log.Panic("Cannot write the json Hello file", err.Error())
	}
}
...
func (backRepoHello *BackRepoHelloStruct) RestorePhaseOne(dirPath string) {
	...
	if err != nil {
		log.Panic("Cannot restore/open the json Hello file", filename, " ", err.Error())
	}
	...
	if err != nil {
		log.Panic("Cannot restore/unmarshall json Hello file", err.Error())
	}
}
