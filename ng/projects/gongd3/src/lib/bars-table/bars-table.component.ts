...
export class BarsTableComponent implements OnInit {
  ...
  ngAfterViewInit() {
    ...
    this.matTableDataSource.filterPredicate = (barDB: BarDB, filter: string) => {
      ...
      if (barDB.X) {
        mergedContent += barDB.X.Name.toLowerCase()
      }
      if (barDB.Y) {
  ...
  displayBarInRouter(barID: number) {
    this.router.navigate(["gongd3_go-" + "bar-display", barID])
  }
  ...
  setEditorRouterOutlet(barID: number) {
    this.router.navigate([{
      outlets: {
        gongd3_go_editor: ["gongd3_go-" + "bar-detail", barID]
      }
    }]);
  }
  ...
  setPresentationRouterOutlet(barID: number) {
    this.router.navigate([{
      outlets: {
        gongd3_go_presentation: ["gongd3_go-" + "bar-presentation", barID]
      }
    }]);
  }
